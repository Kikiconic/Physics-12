...
function fieldAt(point, strength, earth, magnet = { x: .5, y: .5 }, solenoid = false, reversed = false) {
  ...
  const polarity=reversed?-1:1;
  return { x: (bx*Math.cos(angle)-by*Math.sin(angle))*polarity, y: (bx*Math.sin(angle)+by*Math.cos(angle))*polarity };
}
...
export default function MagneticBarSimulator({ variant = "bar" }) {
  ...
  const reading = useMemo(() => {
    const f = fieldAt(meterPos, strength, earth, magnetPos, isSolenoid, reversed);
    return { bx: f.x, by: -f.y, b: Math.hypot(f.x, f.y), theta: Math.atan2(-f.y, f.x) * 180 / Math.PI };
  }, [meterPos, strength, earth, magnetPos, isSolenoid, reversed]);
  ...
  useEffect(() => {
    ...
    const draw = time => {
      ...
      if(isSolenoid){
        ...
        ctx.fillStyle="#fff";ctx.font="800 18px Manrope";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(reversed?"S":"N",-mw*.58,0);ctx.fillText(reversed?"N":"S",mw*.58,0);
        ctx.fillStyle="#8fc9ff";ctx.font="700 9px DM Mono";ctx.fillText("CURRENT-CARRYING COIL",0,mh*.72);
      }else{
        ctx.shadowColor="rgba(0,0,0,.45)";ctx.shadowBlur=18;ctx.fillStyle=reversed?"#d64f59":"#287fc4";ctx.fillRect(-mw/2,-mh/2,mw/2,mh);ctx.fillStyle=reversed?"#287fc4":"#d64f59";ctx.fillRect(0,-mh/2,mw/2,mh);ctx.shadowBlur=0;ctx.strokeStyle="rgba(255,255,255,.72)";ctx.lineWidth=2;ctx.strokeRect(-mw/2,-mh/2,mw,mh);ctx.fillStyle="#fff";ctx.font=`800 ${Math.max(24,mh*.43)}px Manrope`;ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(reversed?"S":"N",-mw/4,0);ctx.fillText(reversed?"N":"S",mw/4,0);
      }
      if(showInside){
        ...
        insideRows.forEach(y=>{
          const insideStart=reversed?-mw*insideReach:mw*insideReach,insideEnd=-insideStart;ctx.beginPath();ctx.moveTo(insideStart,y);ctx.lineTo(insideEnd,y);ctx.stroke();
          const headBack=reversed?insideEnd-.04*mw:insideEnd+.04*mw;ctx.beginPath();ctx.moveTo(insideEnd,y);ctx.lineTo(headBack,y-3.5);ctx.lineTo(headBack,y+3.5);ctx.closePath();ctx.fill();
          if(!live||flow>=.72){const baseX=mw*.42-insideProgress*mw*.84,dotX=reversed?-baseX:baseX;ctx.fillStyle="#fff";ctx.shadowColor="#f7dc72";ctx.shadowBlur=isSolenoid?14:9;ctx.beginPath();ctx.arc(dotX,y,isSolenoid?3.7:3,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;ctx.fillStyle="#f7dc72";}
        });
      }ctx.restore();
      if(showField){for(let y=.08;y<.95;y+=.09)for(let x=.05;x<.97;x+=.075){if(Math.abs(x-magnetPos.x)<.2&&Math.abs(y-magnetPos.y)<.1)continue;const f=fieldAt({x,y},strength,earth,magnetPos,isSolenoid,reversed),m=Math.hypot(f.x,f.y),len=clamp(7+Math.log(m+1)*2,8,17),ang=Math.atan2(f.y,f.x),px=x*w,py=y*h;ctx.save();ctx.translate(px,py);ctx.rotate(ang);ctx.strokeStyle="rgba(143,201,255,.48)";ctx.fillStyle="rgba(143,201,255,.65)";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(-len/2,0);ctx.lineTo(len/2,0);ctx.stroke();ctx.beginPath();ctx.moveTo(len/2,0);ctx.lineTo(len/2-4,-3);ctx.lineTo(len/2-4,3);ctx.closePath();ctx.fill();ctx.restore();}}
      const drawCompass=(p,label)=>{const px=p.x*w,py=p.y*h,f=fieldAt(p,strength,earth,magnetPos,isSolenoid,reversed),ang=Math.atan2(f.y,f.x);ctx.save();ctx.translate(px,py);ctx.fillStyle="rgba(3,16,26,.9)";ctx.strokeStyle="#fff";ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,27,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.rotate(ang);ctx.fillStyle="#ef5b68";ctx.beginPath();ctx.moveTo(22,0);ctx.lineTo(-4,-6);ctx.lineTo(-4,6);ctx.closePath();ctx.fill();ctx.fillStyle="#dbe8ef";ctx.beginPath();ctx.moveTo(-22,0);ctx.lineTo(4,-6);ctx.lineTo(4,6);ctx.closePath();ctx.fill();ctx.restore();ctx.fillStyle="#fff";ctx.font="700 9px DM Mono";ctx.textAlign="center";ctx.fillText(label,px,py+42);};
  ...
  },[showField,showInside,earth,compass,meter,live,strength,compassPos,meterPos,magnetPos,isSolenoid,reversed]);
