...
export default function ElectricFieldBackground() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      time += .0022;
      const positive = { x: width * .64, y: height * .45 };
      const negative = { x: width * .84, y: height * .45 };
      ...
      [
        { ...positive, sign: "+", core: "#69b7ff" },
        { ...negative, sign: "−", core: "#e8f4ff" }
      ].forEach(charge => {
        ...
        ctx.beginPath();
        ctx.arc(charge.x, charge.y, 23, 0, Math.PI * 2);
        ctx.fill();
