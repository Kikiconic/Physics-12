...
const lessons = [
  {n:"01",time:"5 MIN",title:"Electric charge",text:"Positive, negative, conservation, and the rules that determine how charges interact.",symbol:"+ −"},
  {n:"02",time:"6 MIN",title:"Coulomb’s law",text:"Calculate the magnitude and direction of force between two point charges.",symbol:"F"},
  {n:"03",time:"7 MIN",title:"Electric fields",text:"Map the force that a positive test charge would feel at every point in space.",symbol:"E"},
  {n:"04",time:"7 MIN",title:"Electric potential",text:"Connect electric potential energy, voltage, and the motion of charges.",symbol:"V"}
];
  ...
  return (
    <main>
      ...
      <section className="lessons-page-grid">
        {lessons.map(x=><article key={x.n}><div className="lesson-page-top"><span>{x.n}</span><b>{x.symbol}</b></div><small>{x.time}</small><h2>{x.title}</h2><p>{x.text}</p><Link href={x.n==="02"?"/simulators":x.n==="04"?"/formulas":"#lesson-notes"}>Explore topic <ArrowIcon /></Link></article>)}
      </section>
