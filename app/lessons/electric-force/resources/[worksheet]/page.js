...
export function generateStaticParams() {
  return worksheetResources.map((worksheet) => ({ worksheet: worksheet.slug }));
}

export default async function WorksheetResourcePage({ params }) {
  const { worksheet: worksheetSlug } = await params;
  const worksheet = worksheetResources.find((item) => item.slug === worksheetSlug);
  ...
  return (
    <main>
      ...
      <header className="lesson-detail-hero resource-page-hero">
        <div>
          <Link href="/lessons/electric-force/resources">← Additional Practice</Link>
          <span>Worksheet No. {worksheet.number}</span>
        </div>
        <h1>{worksheet.title}</h1>
        <p>Open the worksheet and use the recorded solutions for selected questions.</p>
      </header>
