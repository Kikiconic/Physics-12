...
export function generateStaticParams() {
  return electricPotentialWorksheets.map((worksheet) => ({ worksheet: worksheet.slug }));
}

export default async function ElectricPotentialWorksheetPage({ params }) {
  const { worksheet: worksheetSlug } = await params;
  const worksheet = electricPotentialWorksheets.find((item) => item.slug === worksheetSlug);
  ...
  return (
    <main>
      ...
      <header className="lesson-detail-hero resource-page-hero">
        <div>
          <Link href="/lessons/electric-potentials/resources">← Additional Practice</Link>
